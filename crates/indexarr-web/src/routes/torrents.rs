...
async fn get_torrent(
    ...
    let row = torrent_row.ok_or_else(|| {
        (axum::http::StatusCode::NOT_FOUND, format!("torrent {hash} not found"))
    })?;

    // Fetch content
    let content_row = sqlx::query(
        "SELECT * FROM torrent_content WHERE info_hash = $1"
    )
    .bind(&hash)
    .fetch_optional(pool)
    .await
    ...
    let content = content_row.map(|r| serde_json::json!({
    ...
    // Fetch files
    let file_rows = sqlx::query("SELECT path, size, extension FROM torrent_files WHERE info_hash = $1 ORDER BY path")
        .bind(&hash)
        .fetch_all(pool)
        .await
        .map_err(db_err)?;

    let files: Vec<FileItem> = file_rows.iter().map(|r| FileItem {
        path: r.get("path"),
        size: r.get("size"),
    ...
    // Fetch tags
    let tag_rows = sqlx::query_scalar::<_, String>("SELECT tag FROM torrent_tags WHERE info_hash = $1")
        .bind(&hash)
        .fetch_all(pool)
        .await
        .map_err(db_err)?;
    ...
    Ok(Json(TorrentDetail {
        ...
        resolved_at: row.get::<Option<DateTime<Utc>>, _>("resolved_at").map(|d| d.to_rfc3339()),
